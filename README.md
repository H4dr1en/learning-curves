...
|                        | max_scaling       | int                                        | 2           | Order of magnitude added to the order of magnitude of the maximum train set size.  If `Predictor` is diverging, the total order of magnitude is used for the calculation of the saturation value. Generally, a value of `2` is enough.                                            |
|                        | resolution        | int                                        | 1e4         | The higher, the more accurate will be the optimal training set size (risk of `MemoryException` if `max_scaling` is also high)                                                                                                                                                     |
