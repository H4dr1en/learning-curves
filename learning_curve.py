...
class LearningCurve():
    ...
    def get_lc(self, estimator, X, Y, train_sizes=None, test_size=0.2, n_splits=3, verbose=1, n_jobs=-1, **kwargs):
        ...
        result = self.train(estimator, X, Y, train_sizes=None, test_size=0.2, n_splits=3, verbose=verbose, n_jobs=-1, **kwargs)
        return self.plot_lc(**result, predictor="best")
    ...
    def get_predictor(self, name:str):
        """ This gets the first predictor with matching {name}, and returns None if no predictor matches. """
    ...
    def fit_all(self, x, y):
        """ Fit a curve with all the functions and retrieve r2 score if y_pred is finite.
            Returns an array of tuples (popt,pcov,r2)."""
